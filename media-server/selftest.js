...
await check('unpublish idles viewers during the linger window', async () => {
    publisher.json({ t: 'unpublish', sid: publisherSid });
    const idleState = await viewer.waitText((m) => m.t === 'stream' && m.state === 'idle', 'idle state');
    assert.equal(idleState.reason, 'unpublished');
    assert.equal(idleState.key, STREAM);
});
