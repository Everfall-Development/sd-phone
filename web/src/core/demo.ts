...
// on its own. The phone is not rendered at all in that mode: booting it first
// and opening the panel over it flashes a device the visitor did not ask for,
// and leaves one behind the panel's transparent backdrop.
export const demoAdminOnly: boolean =
    isDemo && typeof location !== 'undefined' && new URLSearchParams(location.search).has('admin');
