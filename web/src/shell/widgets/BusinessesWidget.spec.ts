...
const companies: Company[] = [
    ...
    { id: 'far', name: 'Far Shop', location: 'Retail', category: 'Retail', color: '#fff', emoji: 'F', canCall: true, canMessage: true, emergency: false, status: 'open', coords: { x: 500, y: 0, z: 0 } },
    { id: 'near', name: 'Near Shop', location: 'Retail', category: 'Retail', color: '#fff', emoji: 'N', canCall: true, canMessage: true, emergency: false, status: 'open', coords: { x: 100, y: 0, z: 0 } },
    { id: 'closed', name: 'Closed Shop', location: 'Retail', category: 'Retail', color: '#fff', emoji: 'C', canCall: true, canMessage: true, emergency: false, status: 'closed', coords: { x: 1, y: 0, z: 0 } },
];
...
describe('Businesses widget projections', () => {
    ...
    it('keeps every candidate until after nearest sorting', () => {
        const many = Array.from({ length: 60 }, (_, index): Company => ({
            id: `business-${index}`,
            name: `Business ${index}`,
            location: 'Retail',
            category: 'Retail',
    ...
    it('orders staff threads by unread count and then recency', () => {
        const inbox: Inbox = {
            ...
            hasJob: true,
            job: [
                { key: 'a', name: 'A', color: '#fff', emoji: 'A', preview: '', ts: 100, unread: 1, messages: [] },
                { key: 'b', name: 'B', color: '#fff', emoji: 'B', preview: '', ts: 90, unread: 2, messages: [] },
                { key: 'c', name: 'C', color: '#fff', emoji: 'C', preview: '', ts: 110, unread: 1, messages: [] },
            ],
        };
    ...
    it('does not expose personal threads as a staff queue', () => {
        const inbox: Inbox = {
            personal: [{ key: 'personal', name: 'Personal', color: '#fff', emoji: 'P', preview: '', ts: 1, unread: 3, messages: [] }],
            job: [],
            hasJob: false,
        };
